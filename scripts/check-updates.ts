#!/usr/bin/env tsx
/**
 * Vietnamese Law MCP — Data Freshness Checker
 *
...
const PORTAL_URL = 'https://vbpl.vn';
const PORTAL_NAME = 'VBPL (Van ban phap luat)';
...
async function checkPortal(url: string): Promise<boolean> {
  try {
    ...
    const res = await fetch(url, {
      ...
      headers: { 'User-Agent': '@ansvar/vietnamese-law-mcp/1.0 (data-freshness-check)' },
    });
...
async function main(): Promise<void> {
  console.log('Vietnamese Law MCP — Data Freshness Check');
