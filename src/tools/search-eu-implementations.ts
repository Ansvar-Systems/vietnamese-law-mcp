/**
 * search_eu_implementations — Search EU directives/regulations referenced by Vietnamese legislation.
 */
...
export interface SearchEUImplementationsInput {
  ...
  year_from?: number;
  year_to?: number;
  has_vietnamese_implementation?: boolean;
  limit?: number;
}
...
export interface EUImplementationSearchResult {
  ...
  vietnamese_statute_count: number;
}
...
export async function searchEUImplementations(
  ...
  let sql = `
    SELECT
      ...
      ed.title,
      ed.short_name,
      COUNT(DISTINCT er.document_id) as vietnamese_statute_count
  ...
  if (input.has_vietnamese_implementation) {
    sql += ' HAVING vietnamese_statute_count > 0';
  }
